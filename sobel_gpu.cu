...
sobel_filtered_pixel(float *s, int i, int j, int ncols, int nrows, float *gx, float *gy)
{
   ...
   // Loop over the 3x3 window centered at (i, j)
   for (int di = -1; di <= 1; di++)
   {
      for (int dj = -1; dj <= 1; dj++)
      {
         ...
         // Check if (ni, nj) is within the bounds of the image
         if (ni >= 0 && ni < nrows && nj >= 0 && nj < ncols)
         {
            float pixel = s[ni * ncols + nj];
            int idx = (di + 1) * 3 + (dj + 1); // Mapping 2D to linear index for 3x3 filter
            Gx += gx[idx] * pixel;
            Gy += gy[idx] * pixel;
         }
      }
...
#define ONE_OVER_255 0.003921568627451
   ...
   int nThreadsPerBlock = atoi(av[1]);
   int nBlocks = atoi(av[2]);

   // Check if we have the correct number of arguments (expecting 3: executable name, num_threads_per_block, num_blocks)
   if (ac != 3)
   {
      printf("Usage: %s <num_threads_per_block> <num_blocks>\n", av[0]);
      return 1;
   }
